...
Variant data can be downloaded here: https://elegansvariation.org/data/release/latest
...
stats_soft_filter <- readLines(glue::glue("WI.{vcf_date}.soft-filter.stats.txt"))
SN_soft <- stats_soft_filter[c(grep(pattern = "^SN", x = stats_soft_filter))]
SN_soft_df <- read.table(text = SN_soft, sep = "\t")

stats_hard_filter <- readLines(glue::glue("WI.{vcf_date}.hard-filter.stats.txt"))
...
filter_stats <- read.delim(glue::glue("WI.{vcf_date}.soft-filter.filter_stats.txt"), header=FALSE, stringsAsFactors=FALSE) 
