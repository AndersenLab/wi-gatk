...
pa <- ggplot2::ggplot() + 
    ...
    ggplot2::geom_text(data=filter_thresh, aes(x = Inf, y = Inf,
              ...
              colour="red",
              size = 4) + 
    ggplot2::xlab("Variant quality score (QUAL)") + 
...
pb <- ggplot2::ggplot() + 
    ...
    ggplot2::geom_text(data=filter_thresh, aes(x = Inf, y = Inf,
              ...
              colour="red",
              size = 4) + 
    ggplot2::xlab("QUAL normalized by read depth (QD)") + 
    ggplot2::ylab("Number of sites") +
    ggplot2::theme_bw() +
    presentation +
    ggplot2::scale_y_continuous(labels = scales::comma)
...
pc <- ggplot2::ggplot() + 
    ...
    ggplot2::geom_text(data=filter_thresh, aes(x = Inf, y = Inf,
              ...
              colour="red",
              size = 4) + 
    ggplot2::xlab("Strand bias: strand odds ratio (SOR)") + 
    ggplot2::ylab("Number of sites") +
    ggplot2::theme_bw() +
    presentation +
    ggplot2::scale_y_continuous(labels = scales::comma)
...
pd <- ggplot2::ggplot() + 
    ...
    ggplot2::geom_text(data=filter_thresh, aes(x = Inf, y = Inf,
              ...
              colour="red",
              size = 4) + 
    ggplot2::xlab("Strand bias: Fisher strand (FS)") + 
    ggplot2::ylab("Number of sites") +
    ggplot2::theme_bw() +
    presentation + 
...
DT::datatable(dplyr::select(PSC_soft_df, sample, total_SNV, nHets, het_SNV_rate, nIndels, nHetIndels, het_Indel_rate, nSingletons, nMissing), 
...
DT::datatable(dplyr::select(PSC_hard_df, sample, total_SNV, nHets, het_SNV_rate, nIndels, nHetIndels, het_Indel_rate, nSingletons, nMissing), 
              colnames = c("Strain", "Total SNVs", "Het SNVs", "Het SNVs/Total SNVs", "Total Indels", "Het Indels", "Het Indels/Total Indels", "Private SNVs and Indels", "Missing"),
              rownames = F,
              options = list(pageLength = 5000, scrollY = 400, dom = 'ft'),
              extensions = c("Scroller"))
