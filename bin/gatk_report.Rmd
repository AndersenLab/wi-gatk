...
presentation <- ggplot2::theme(axis.text.x = element_text(size=10, face="bold", color="black"),
...
filter_stats %<>% 
    dplyr::mutate( QUAL=as.numeric(QUAL), QD=as.numeric(QD), SOR=as.numeric(SOR), FS=as.numeric(FS))
...
filter_stats %<>%
    dplyr::mutate(FILTER= stringr::str_replace_all(FILTER, "SOR_strand_odds_ratio","SOR")) %>% 
    dplyr::mutate(FILTER= stringr::str_replace_all(FILTER, "QD_quality_by_depth","QD")) %>% 
    dplyr::mutate(FILTER= stringr::str_replace_all(FILTER, "high_heterozygosity","high_het")) %>% 
    dplyr::mutate(FILTER= stringr::str_replace_all(FILTER, "FS_fisherstrand","FS")) %>%
    dplyr::mutate(FILTER= stringr::str_replace_all(FILTER, "QUAL_quality","QUAL")) %>% 
    dplyr::mutate(FILTER= stringr::str_split(FILTER, ";")) 

filter_stats %>% 
    ggplot2::ggplot(aes(x=FILTER)) + 
    ggplot2::geom_bar() + 
    ggplot2::scale_x_upset(n_intersections = 20) + 
...
split_header <- stringr::str_replace_all(PSC_header,"[:digit:]|\\[|\\]","") %>% 
    strsplit("\t")
...
split_header <- stringr::str_replace_all(PSI_header,"[:digit:]|\\[|\\]","") %>% 
    strsplit("\t")
...
DT::datatable(select(PSC_soft_df, sample, total_SNV, nHets, het_SNV_rate, nIndels, nHetIndels, het_Indel_rate, nSingletons, nMissing), 
...
p01 <- ggplot2::ggplot(PSC_soft_df) + 
    ...
    ggplot2::theme_bw() +
    presentation  + 
    ggplot2::xlab("Het SNVs/Total SNVs") + 
...
p02 <- ggplot2::ggplot(PSC_soft_df) + 
    ...
    ggplot2::theme_bw() +
    presentation  + 
    ggplot2::xlab("Het Indels/Total Indels") + 
    ggplot2::ylab("Number of strains") +
    ggplot2::scale_y_continuous(labels = scales::comma)
...
DT::datatable(select(PSC_hard_df, sample, total_SNV, nHets, het_SNV_rate, nIndels, nHetIndels, het_Indel_rate, nSingletons, nMissing), 
              colnames = c("Strain", "Total SNVs", "Het SNVs", "Het SNVs/Total SNVs", "Total Indels", "Het Indels", "Het Indels/Total Indels", "Private SNVs and Indels", "Missing"),
              rownames = F,
              options = list(pageLength = 5000, scrollY = 400, dom = 'ft'),
              extensions = c("Scroller"))
